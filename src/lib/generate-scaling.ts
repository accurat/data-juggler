...
function scaleFromMoment(m: MomentsType): ScalingFn {
  const { min, max } = m;
  return (lower: number, upper: number) => (scaled: unknown) =>
    !(isNull(max) || isNull(min))
      ? (toNumber(scaled) * (max - min) + min - lower) / (upper - lower)
      : toNumber(scaled);
}

export function scalesFromMoments<T extends StringKeyedObj>(
  ...
  return reduce(
    ...
    (acc, variable) => ({
      ...acc,
      [variable]: scaleFromMoment(moments[variable])
    }),
    {} as ScalingFnsRecords<T>
  );
}
